...
public class AddrServiceImplV1 implements AddrService{
	 ...
	 * 객체를 선언하고, 클래스의 생성자를 통해서 객체를 초기화 하는
	 ...
	 * 준비된 객체를 주입해주는 기능 
	 * 
	 * 의존성 주입하는 방법은 필드주입, 생성자주입, Setter주입이 있다
	 */
	 ...
	 * 또 다시 주입을 할 수 없다. 객체 보호가 완전하다
	 * 객체의 순환참조가 없다
	 * 이 방식은 메모리 누수가 없고, 객체가 보호되며, 순환참조에 의한
	 * 코드의 exception 을 막는다
	 * @Autowired 를 붙이지 않아도 된다
	 * 
	 * 이 방식을 사용하려면 반드시 생성자 메서드가 있어야 한다
	 * 코드가 다소 번잡해 진다
	 * 실수로 생성자 메서드를 만들지 않았을 경우 NullPointer exception 이
	 * 발생한다
	 */
