...
document.addEventListener("DOMContentLoaded", () => {
   ...
   *  1. 변수 1의 값이 false 가 아니면 변수 1의 값을 왼쪽 변수에 대입
   *  2. 변수 2의 값이 false 이면 변수 2의 값을 비교
   *  3. 변수 2의 값이 false 가 아니면 변수 2의 값을 왼쪽 변수에 대입
   *  4. 변수 4까지 (변수1 ~ 변수4) 모든 변수가 false 인 값을 가지고 있으면
  ...
  // inline 익명(무명) 함수로 click evnet 설정
  btn_list?.addEventListener("click", (e) => {
    // alert("리스트 버튼 클릭");
    // 현재 화면에 보여줄 페이지 요청하기
    //! js 에서 사용하기 위해 jsp에서 rootPath 변수로 재 정의했음
  ...
  input_id?.addEventListener("blur", async (e) => {
    ...
    if (!value) {
      message_view(INDEX.ID, "error", "* ID 를 입력해 주세요");
      input_id.focus();
      return false;
    }
    ...
    if (result === "OK") {
      ...
      message_view(INDEX.ID, "error", "* 중복된 아이디 입니다. 사용불가!!");
      input_id.focus();
    } else {
