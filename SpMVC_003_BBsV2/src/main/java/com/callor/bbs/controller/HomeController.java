...
public class HomeController {
	 ...
	 * 주입받아 사용해야 하는지 알 수가 없다
	 * 
	 * 이때는 @Component("이름") 에 이름을 부여하고
	 * 사용할 곳에서 주입받을 때 @Qualifier() 를 통해 명시적으로
...
//		List<BBsDto> bbsList = bbsDao.selectAll();
	 ...
	 * method 를 찾는다
	 * 만약 method가 발견되면 자동으로 해당 method 를 호출하여
	 * 데이터를 준비해 준다
	 */
...
//		model.addAttribute("BBS",bbsDto);
	 ...
	 * 매개변수 설명
	 * BBsDto bbsDto 
	 * 	: input tag 의 입력한 게시판관련 text 를 받는 곳
	...
	public String insert(
		...
		try {
			...
			// TODO Auto-generated catch block
			 e.printStackTrace();
			log.debug("파일을 업로드 할 수 없음. 오류 발생!!");
